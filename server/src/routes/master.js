...
router.get("/suppliers", (req, res) => {
  const rows = db
    .prepare(
      `SELECT s.*, (SELECT COUNT(*) FROM tbl_product p WHERE p.brand = s.supplier_name AND p.is_archived = 0) AS item_count
       FROM tbl_supplier s WHERE s.is_archived = 0 ORDER BY s.supplier_name`
    )
    .all();
  res.json(rows);
});
...
router.post("/suppliers", (req, res) => {
  ...
  const exists = db
    .prepare("SELECT sup_id FROM tbl_supplier WHERE supplier_name = ? AND is_archived = 0")
    .get(String(supplier_name).trim());
  if (exists) return res.status(400).json({ error: "Supplier already exists." });
  const info = db
    .prepare("INSERT INTO tbl_supplier (supplier_name, contact, address, is_archived) VALUES (?, ?, ?, 0)")
    .run(String(supplier_name).trim(), contact || "", address || "");
  log(req.user.userid, `Added Supplier: ${supplier_name}`);
  res.status(201).json({ sup_id: Number(info.lastInsertRowid) });
});

router.put("/suppliers/:id", (req, res) => {
  const s = db.prepare("SELECT * FROM tbl_supplier WHERE sup_id = ?").get(req.params.id);
  if (!s) return res.status(404).json({ error: "Supplier not found" });
  const { supplier_name, contact, address } = req.body || {};
  db.prepare("UPDATE tbl_supplier SET supplier_name = ?, contact = ?, address = ? WHERE sup_id = ?").run(
    supplier_name !== undefined && String(supplier_name).trim() ? String(supplier_name).trim() : s.supplier_name,
    contact !== undefined ? contact : s.contact,
    address !== undefined ? address : s.address,
    s.sup_id
  );
  log(req.user.userid, `Updated Supplier: ${s.supplier_name}`);
  res.json({ success: true });
});
...
router.delete("/suppliers/:id", (req, res) => {
  const s = db.prepare("SELECT * FROM tbl_supplier WHERE sup_id = ?").get(req.params.id);
  if (!s) return res.status(404).json({ error: "Supplier not found" });
  db.prepare("UPDATE tbl_supplier SET is_archived = 1 WHERE sup_id = ?").run(s.sup_id);
  log(req.user.userid, `Deleted Supplier: ${s.supplier_name}`);
  res.json({ success: true });
});
